...
public class FlightService {
	...
	public Map<String,List<SearchResponse>> searchFlightDetails(SearchDetails details) throws AdminException{
		...
		if(details.getSource() != null && details.getDestination() != null && details.getJourney_date() != null && details.getSeat() != null && details.getClass_type() != null && details.getSeat() != 0) {
			Map<String,List<SearchResponse>> map = new HashMap<>();
			if(details.getClass_type().equalsIgnoreCase("bussiness")) {
				
				List<SearchResponse> response = repo.searchBusFlight(details.getSource(),details.getDestination(),details.getJourney_date(),details.getSeat());
				...
				List<SearchResponse> response =repo.searchNonBusFlight(details.getSource(),details.getDestination(),details.getJourney_date(),details.getSeat());
				map.put("OneWay", response);
				return map;
			}
		...
		throw new AdminException("Incorrect value recived");
			...
			if(details.getSource() != null && details.getDestination() != null && details.getJourney_date() != null && details.getSeat() != null && details.getClass_type() != null && details.getSeat() != 0 && details.getReturn_destination() != null && details.getReturn_source() != null && details.getReturn_date() != null) {
				Map<String,List<SearchResponse>> map = new HashMap<>();
				if(details.getClass_type().equalsIgnoreCase("bussiness")) {
					
					List<SearchResponse> response = repo.searchBusFlight(details.getSource(),details.getDestination(),details.getJourney_date(),details.getSeat());
					map.put("OneWay", response);
					List<SearchResponse> response1 = repo.searchBusReFlight(details.getReturn_source(),details.getReturn_destination(),details.getReturn_date(),details.getSeat());
					...
					List<SearchResponse> response = repo.searchNonBusFlight(details.getSource(),details.getDestination(),details.getJourney_date(),details.getSeat());
					map.put("OneWay", response);
					List<SearchResponse> response1 = repo.searchNonBusReFlight(details.getReturn_source(),details.getReturn_destination(),details.getReturn_date(),details.getSeat());
					map.put("TwoWay", response1);
					return map;
				}
				throw new AdminException("Incorrect value recived");
	...
	public String blockFlight(FlightDetails details) throws AdminException{
		if((Integer)details.getId() != null && details.getComment() != null) {
			Optional<FlightDetails> optional = repo.findById(details.getId());
			if(optional.isPresent()) {
				FlightDetails flight = optional.get();
				LocalDateTime modifydate = LocalDateTime.now();
				...
				FlightDetails flight1 = repo.save(flight);
				return "Flight id:- "+details.getId()+" is blocked";
			} else {		
				throw new AdminException("Flight id:- "+details.getId()+" is not present");
				}
		}
	...
	public String unblockFlight(FlightDetails details)  throws AdminException{
	if((Integer)details.getId() != null && details.getComment() != null) {
		Optional<FlightDetails> optional = repo.findById(details.getId());
		if(optional.isPresent()) {
			FlightDetails flight = optional.get();
			...
			flight.setComment(details.getComment());
			FlightDetails flight1 = repo.save(flight);
			return "Flight id:- "+details.getId()+" is unblocked";
		} else {		
			throw new AdminException("Flight id:- "+details.getId()+" is not present");
		}
		}
